...
{
	AkSpeakerVolumes * pVolumesDry[ AK_VOICE_MAX_NUM_CHANNELS ];	///< For each channel in the sound, pointer to AkSpeakerVolume describing contribution to destination channels. 
	AkUInt32 uChannelMask;											///< Speaker setup of the sound (for example, AK_SPEAKER_SETUP_STEREO)
	AkReal32 * io_pfDryAttenuation;									///< Attenuation to apply on the Dry Attenuation.
	AkReal32 * io_pfGameDefAuxSendAttenuation;						///< Attenuation to apply on the Game defined auxilliary send attenuation.
	AkReal32 * io_pfUserDefAuxSendDryAttenuation;					///< Attenuation to apply on the User defined auxilliary send attenuation.
};
